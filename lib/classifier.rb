# usage: ruby classifier.rb /full/path/input/file.txt.gz /full/path/output/file.txt
require 'zlib'
require 'json'
  ...
  gzi.each_line do |line|
    begin
      ...
      if parsed_line["user"]["followers_count"].to_i >= 1000000 or parsed_line["user"]["friends_count"] >= 1000000
        response = %x(/usr/bin/python /local/scratch/szuhg2/stcs/botornot-python/botornot.py @#{parsed_line["user"]["screen_name"]})
        parsed_response = JSON.parse(response)
        if parsed_response["score"].to_f >= 0.33 #bot = 1, notbot/human = 0
